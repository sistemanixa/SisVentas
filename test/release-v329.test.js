...
const coreInmutable = fs.readFileSync('js/core/version.v2.0.329.js', 'utf8');
...
test('v2.0.329 publica archivos inmutables y marcadores consistentes', () => {
  assert.match(html, /VERSION:\s*'v2\.0\.329-firebase'/);
  assert.match(html, /js\/app\.v2\.0\.329\.js/);
  assert.match(html, /js\/core\/version\.v2\.0\.329\.js/);
  assert.match(html, /id="loading-version"[^>]*>v2\.0\.329</);
  assert.match(html, /css\/app\.css\?v=2\.0\.329/);
  assert.match(app, /VERSION:\s*'v2\.0\.329-firebase'/);
  assert.match(app, /version:\s*'v2\.0\.329'/);
  assert.match(core, /v2\.0\.329/);
  assert.match(coreInmutable, /v2\.0\.329/);
  assert.match(sw, /sisventas-v2\.0\.329/);
  assert.match(sw, /app\.v2\.0\.329\.js/);
});
...
test('Novedades registra obligatoriamente v2.0.329', () => {
  assert.match(app, /RELEASE_HISTORY[\s\S]*?version:\s*'v2\.0\.329'/);
  assert.match(app, /Métricas y rendimiento de OT/);
});
