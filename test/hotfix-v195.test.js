...
const appVersionada = fs.readFileSync(path.join(root, 'js', 'app.v2.0.200.js'), 'utf8');
const index = fs.readFileSync(path.join(root, 'index.html'), 'utf8');

test('producción carga exactamente el código validado de v2.0.200', () => {
  assert.equal(appVersionada, app);
  assert.match(index, /js\/app\.v2\.0\.200\.js/);
  assert.match(index, /js\/core\/version\.v2\.0\.200\.js/);
});
