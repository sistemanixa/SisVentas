...
const app = fs.readFileSync(path.join(root, 'js', 'app.v2.0.282.js'), 'utf8');
...
test('la publicación v2.0.282 mantiene todas las referencias coherentes', () => {
  assert.match(index, /app\.v2\.0\.282\.js/);
  assert.match(index, /version\.v2\.0\.282\.js/);
  assert.match(worker, /sisventas-v2\.0\.282/);
  assert.match(publishedVersion, /SISVENTAS_PWA_VERSION\s*=\s*'v2\.0\.282'/);
  assert.match(app, /VERSION:\s*'v2\.0\.282-firebase'/);
});
