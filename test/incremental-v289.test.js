...
const app = fs.readFileSync(path.join(root, 'js', 'app.v2.0.289.js'), 'utf8');
const worker = fs.readFileSync(path.join(root, 'sw.js'), 'utf8');
const metrics = fs.readFileSync(path.join(root, 'js', 'modules', 'sales-metrics.js'), 'utf8');

test('la publicacion v2.0.289 usa referencias coherentes', () => {
  assert.match(index, /app\.v2\.0\.289\.js/);
  assert.match(index, /version\.v2\.0\.289\.js/);
  assert.match(index, /sales-metrics\.js\?v=2\.0\.289/);
  assert.match(worker, /sisventas-v2\.0\.289/);
});
