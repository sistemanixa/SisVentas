...
const app = fs.readFileSync(path.join(root, 'js', 'app.v3.0.10.js'), 'utf8');
const version = fs.readFileSync(path.join(root, 'js', 'core', 'version.v3.0.10.js'), 'utf8');
...
test('todas las etiquetas visibles consumen la versión central', () => {
  assert.match(version, /window\.SISVENTAS_PWA_VERSION = 'v3\.0\.10'/);
