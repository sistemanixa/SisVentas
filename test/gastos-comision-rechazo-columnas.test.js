...
assert(!grids.includes("if (!preset && table && table.id === 'gas-tbl')"), 'Gastos no debe tener un recomendado paralelo a la regla general.');
assert(!grids.includes("if (!table || table.id !== 'gas-tbl')"), 'Gastos debe reservar Acciones con la misma sanitización general.');
