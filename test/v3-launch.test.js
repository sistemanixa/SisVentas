...
test('V3 publica recursos inmutables y carga el lanzamiento antes de la app', () => {
  ...
  assert.match(html, /VERSION: 'v3\.0\.7-firebase'/);
  assert.match(html, /js\/core\/version\.v3\.0\.7\.js/);
  assert.match(html, /js\/app\.v3\.0\.7\.js/);
  assert.ok(html.indexOf('js/modules/v3-launch.js') < html.indexOf('js/app.v3.0.7.js'));
});
...
test('el service worker precarga el lanzamiento completo de V3', () => {
  const sw = read('sw.js');
  assert.match(sw, /sisventas-v3\.0\.7/);
  assert.match(sw, /app\.v3\.0\.7\.js/);
  assert.match(sw, /version\.v3\.0\.7\.js/);
