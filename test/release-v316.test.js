...
const app = fs.readFileSync('js/app.v2.0.316.js', 'utf8');
const core = fs.readFileSync('js/core/version.js', 'utf8');
const sw = fs.readFileSync('sw.js', 'utf8');
const cotizador = fs.readFileSync('cotizador/index.js', 'utf8');

test('v2.0.316 references the immutable application and version files', () => {
  assert.match(html, /js\/app\.v2\.0\.316\.js/);
  assert.match(html, /js\/core\/version\.v2\.0\.316\.js/);
  assert.match(app, /VERSION:\s*'v2\.0\.316-firebase'/);
  assert.match(core, /v2\.0\.316/);
  assert.match(sw, /sisventas-v2\.0\.316/);
  assert.match(sw, /app\.v2\.0\.316\.js/);
});

test('v2.0.316 keeps the actualizador controls, titles and visible final actions', () => {
  ...
  assert.match(app, /version:\s*'v2\.0\.316'[\s\S]{0,800}Actualizador: ventana adaptable sin contenido recortado/);
