...
La auditoría estructural inicial realizada sobre la antigua base `v2.0.194`
encontró:
...
Esos valores históricos explican por qué corregir un módulo podía alterar otro y por qué
...
- `product-provider-read-model.js`: excluye mano de obra, valida la relación
  técnica con el maestro de proveedores, exige coherencia entre nombre y dominio
  y calcula la vigencia de cada proveedor sin heredar fechas entre enlaces.
...
4. Productos, proveedores y actualización de precios. El modelo de lectura y
   su auditoría ya están implementados; resta comparar sus resultados contra la
   pantalla actual antes de habilitar escrituras.
