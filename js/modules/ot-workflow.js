(function(){
  ...
  function show(step){
    ...
    if(contexto){
      var fechaHora=ot&&ot.fecha ? String(ot.fecha).split('-').reverse().join('/')+(ot.hora?' · '+esc(ot.hora):'') : 'Sin fecha';
      contexto.innerHTML='<strong style="font-family:monospace;font-size:13px;color:var(--blue)">'+esc(ot&&ot.id||'OT')+'</strong><span><i class="ti ti-user" style="color:var(--text3)"></i> <strong>'+esc(ot&&ot.cliente||'Sin cliente')+'</strong></span><span><i class="ti ti-user-cog" style="color:var(--text3)"></i> '+esc(ot&&ot.tecnico||'Sin técnico')+'</span><span><i class="ti ti-calendar" style="color:var(--text3)"></i> '+fechaHora+'</span><span style="margin-left:auto">'+(typeof otBadge==='function'?otBadge(ot||{}):'')+'</span>';
    }
